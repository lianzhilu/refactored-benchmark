// 700.fusion-webshop-claude-middle/712.listrecommendations/nodejs/function.js
...
async function handleBusinessLogic(event, callFunction) {
    ...
    // 方案2：优先使用调用方（frontend）传入的产品列表，避免重复调用 listproducts
    // 若调用方未传入（向后兼容），则回退到自行调用 listproducts
    let productList;
    if (event.products && Array.isArray(event.products) && event.products.length > 0) {
        productList = event.products;
    } else {
        const fetched = await callFunction("listproducts", {});
        productList = fetched.products;
    }

    let goodProducts = productList.filter(e => event.productIds.includes(e.id));
    let goodProductIds = goodProducts.map(e => e.id);
    let categories = goodProducts.map(e => e.categories);
    categories = categories.flat();

    let otherProducts = productList.filter(e => {
    ...
    // 方案3：只返回 recommendations 字段，移除完整 listproducts 数据（11个产品对象），
    // 减少响应 Payload 约 60~70%
    return {
