// 700.fusion-webshop-claude-middle/714.searchproducts/nodejs/function.js
// 优化方案：方案1（HTTP Keep-Alive）、方案3（消除冗余响应数据 listproducts 字段）、方案7（消除二次JSON解析）
...
async function handleBusinessLogic(event, callFunction) {
    ...
    // 方案3：只返回 products 筛选结果，移除 listproducts 完整列表（冗余数据）
    return {
