// 700.fusion-webshop-claude-middle/709.getcart/nodejs/function.js
// 优化方案：方案1（HTTP Keep-Alive）、方案3（消除冗余响应数据）、方案7（消除二次JSON解析）
...
async function handleBusinessLogic(event, callFunction) {
    ...
    // 方案3：只返回 cart 字段，移除 cartkvstorage 原始响应（DynamoDB原始格式），
    // 减少响应 Payload 约 30~40%，消除调用方的多层兼容性判断
    return {
        cart: cart
    };
}
