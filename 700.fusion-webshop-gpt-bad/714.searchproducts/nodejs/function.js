...
async function handleBusinessLogic(event, callFunction) {
    ...
    if (products.searchableProducts) {
        const matchedIds = products.searchableProducts
            .filter(p => p.searchText.includes(query))
            .map(p => p.id);
        filteredProducts = matchedIds.map(id => products.productsById[id]);
    } else {
        filteredProducts = products.products.filter(p => p.name.toLowerCase().includes(query) || p.description.toLowerCase().includes(query));
    }
