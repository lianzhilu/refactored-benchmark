// 700.fusion-webshop/712.listrecommendations/nodejs/function.js
...
async function handleBusinessLogic(event, callFunction) {
    ...
    let inputIds = new Set(event.productIds || []);
    let goodProducts = products.products.filter(e => inputIds.has(e.id));
    let goodProductIds = new Set(goodProducts.map(e => e.id));
    let categories = new Set(goodProducts.flatMap(e => e.categories));

    let otherProducts;
    if (products.productsByCategory) {
        const candidateIds = new Set();
        for (const category of categories) {
            const ids = products.productsByCategory[category] || [];
            for (const id of ids) {
                if (!goodProductIds.has(id)) {
                    candidateIds.add(id);
                }
            }
        }
        otherProducts = Array.from(candidateIds).map(id => products.productsById[id]);
    } else {
        otherProducts = products.products.filter(e => {
            if (goodProductIds.has(e.id)) {
                return false;
            }
            return e.categories.some(cat => categories.has(cat));
        });
    }

    let recommendations = otherProducts.sort(() => 0.5 - Math.random()).slice(0,2);
