...
exports.handler = async function(event) {
    try {
        ...
        // 并行拉取独立依赖，缩短 checkout 关键路径。
        let [cart, productsList] = await Promise.all([
        ...
        const convertedPriceCache = Object.create(null);

        let orderProducts = await Promise.all(cartItems.map(async item => {
            let itemId = item.itemId?.S || item.itemId || item.id;
            let pr = productsById[itemId];
            if (!pr) return null;

            const cacheKey = `${itemId}:${currencyPref}`;
            let newPrice = convertedPriceCache[cacheKey];
            if (!newPrice) {
                newPrice = await invokeFunctionViaProxy("currency", {
                    from: pr.priceUsd,
                    toCode: currencyPref
                });
                convertedPriceCache[cacheKey] = newPrice;
            }
