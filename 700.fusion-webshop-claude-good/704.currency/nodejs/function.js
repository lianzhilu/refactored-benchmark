...
// 方案2: 复用 HTTP 连接，避免每次 RPC 重新 TCP 握手
 ...
 *
 * 方案1: 新增批量模式
 *   - 批量模式: event.items 为数组，每项格式 { from: {currencyCode, units, nanos}, toCode }
 *     返回: { results: [{units, nanos, currencyCode}, ...] }
 *   - 单价模式（向后兼容）: event = { from: {currencyCode, units, nanos}, toCode }
 *     返回: { units, nanos, currencyCode }
 */
async function handleBusinessLogic(event, callFunction) {
    console.log("currency", event);

    // 方案1: 批量模式 - 一次 RPC 处理多个价格转换，避免 N 次网络往返
    if (Array.isArray(event.items)) {
        const results = event.items.map(item => {
            const rate = getRate(item.from.currencyCode, item.toCode);
            const [convUnits, convNanos] = applyRate(item.from.units, item.from.nanos, rate);
            return { units: convUnits, nanos: convNanos, currencyCode: item.toCode };
        });
        return { results };
    }
