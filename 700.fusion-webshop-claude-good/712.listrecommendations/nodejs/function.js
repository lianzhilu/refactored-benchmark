...
async function handleBusinessLogic(event, callFunction) {
    ...
    // 方案8: 使用 Set 替代 Array，将成员判断从 O(n) 线性扫描优化为 O(1) 哈希查找
    // 构造 goodProductIds 的 Set，用于快速排除已在购物车的商品
    const goodProductIdsSet = new Set(goodProducts.map(e => e.id));
    // 构造 categories 的 Set，用于快速判断分类匹配
    const categoriesSet = new Set(goodProducts.flatMap(e => e.categories));

    let otherProducts = products.products.filter(e => {
        // O(1) 哈希查找替代 O(n) Array.includes
        if (goodProductIdsSet.has(e.id)) {
            return false;
        }
        // O(1) Set.has 替代 O(n) Array.includes
        return e.categories.some(cat => categoriesSet.has(cat));
    });
