...
// 方案2: 进程级 HTTP Keep-Alive Agent，复用 TCP 连接，消除每次 RPC 的握手开销
...
exports.handler = async function(event) {
    try {
        ...
        // 方案1: 批量货币转换 - 将购物车内每个商品的 currency 调用合并为 1 次 RPC
        // 先找出购物车中有效的商品及其价格，批量发出一次转换请求
        const cartProductPairs = cartItems.map(item => {
            const itemId = item.itemId?.S || item.itemId || item.id;
            const pr = products.find(pr => pr.id == itemId);
            return { item, pr, itemId };
        }).filter(pair => pair.pr !== null && pair.pr !== undefined);

        let orderProducts = [];
        if (cartProductPairs.length > 0) {
            // 构造批量转换请求
            const batchItems = cartProductPairs.map(pair => ({
                from: pair.pr.priceUsd,
                toCode: currencyPref
            }));
            const batchResult = await invokeFunctionViaProxy("currency", { items: batchItems });

            // 将批量结果回填到商品对象
            orderProducts = cartProductPairs.map((pair, idx) => {
                const newPrice = batchResult.results[idx];
                pair.pr.price = newPrice;
                return pair.pr;
            });
        }
