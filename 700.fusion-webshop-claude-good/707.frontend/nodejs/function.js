...
async function handleBusinessLogic(event, callFunction, callFunctionAsync) {
    ...
    switch (operation) {
        case "get":
            ...
            // 方案1: 批量货币转换 - 将 N 次独立 currency RPC 合并为 1 次，消除 (N-1) 次网络往返
            const batchCurrencyItems = productsList_response.products.map(pr => ({
                from: pr.priceUsd,
                toCode: currencyPref
            }));
            const batchCurrencyResult = await callFunction("currency", { items: batchCurrencyItems });

            let productsListCurrency = productsList_response.products.map((pr, idx) => {
                const newPrice = batchCurrencyResult.results[idx];
                pr.price = {
