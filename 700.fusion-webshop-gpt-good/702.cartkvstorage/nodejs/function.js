...
async function batchDeleteCartItems(userId, itemIds) {
    for (let index = 0; index < itemIds.length; index += 25) {
        const chunk = itemIds.slice(index, index + 25);
        await ddb.batchWriteItem({
            RequestItems: {
                [CartTable]: chunk.map((itemId) => ({
                    DeleteRequest: {
                        Key: {
                            userId: { S: userId },
                            itemId: { S: itemId }
    ...
    try {
        ...
        if (operation == "empty") {
            let current = await ddb.query({
                ...
                ExpressionAttributeValues: { ":sid": { S: userId } },
                ProjectionExpression: "itemId"
            }).promise()

            current = current.Items.map(item => item.itemId.S);
            if (current.length > 0) {
                await batchDeleteCartItems(userId, current);
            }
