...
exports.handler = async function(event) {
    try {
        ...
        const [cart, productsList] = await Promise.all([
            invokeFunctionViaProxy("getcart", {userId: userId}),
            invokeFunctionViaProxy("listproducts", {})
        ]);
        ...
        if (!Array.isArray(cartItems) && cart.cart && Array.isArray(cart.cart.cart)) {
        ...
        const shipmentPromise = invokeFunctionViaProxy("shipmentquote", {userId: userId, items: cartItems});

        let orderProducts = await Promise.all(cartItems.map(async item => {
            let itemId = item.itemId?.S || item.itemId || item.id;
            let pr = productById.get(String(itemId));
            if (!pr) return null;
        ...
        let shipmentPrice = await shipmentPromise;
