...
const DEBUG_LOGS = process.env.DEBUG_WEBSHOP === '1';
const parsedWeightsCache = new Map();
...
function selectOperationByWeights(weightsStr) {
    let parsed = parsedWeightsCache.get(weightsStr);
    if (!parsed) {
        const operations = [];
        const cumulativeWeights = [];
        let totalWeight = 0;

        for (const item of weightsStr.split(',')) {
            const [op, weight] = item.split(':');
            totalWeight += parseInt(weight.trim(), 10);
            operations.push(op.trim());
            cumulativeWeights.push(totalWeight);
        }

        parsed = { operations, cumulativeWeights, totalWeight };
        parsedWeightsCache.set(weightsStr, parsed);
    }

    const randomValue = Math.random() * parsed.totalWeight;
    let selectedOperation = parsed.operations[0];

    for (let i = 0; i < parsed.operations.length; i++) {
        if (randomValue <= parsed.cumulativeWeights[i]) {
            selectedOperation = parsed.operations[i];
            break;
        }
    }
...
async function handleBusinessLogic(event, callFunction, callFunctionAsync) {
    ...
    switch (operation) {
        case "get":
            ...
            let productIds = new Array(get_cart.length);
            for (let i = 0; i < get_cart.length; i++) {
                productIds[i] = get_cart[i].id || get_cart[i].itemId;
            }
            let recommendations = await callFunction("listrecommendations", {productIds: productIds});
