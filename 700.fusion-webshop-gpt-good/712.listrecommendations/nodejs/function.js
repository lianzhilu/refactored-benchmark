...
function pickUpToTwoRandom(items) {
    if (items.length <= 2) {
        return items.slice();
    }
...
async function handleBusinessLogic(event, callFunction) {
    ...
    const requestedIds = new Set(event.productIds || []);
    const ownedIds = new Set();
    const categorySet = new Set();

    for (const product of products.products) {
        if (!requestedIds.has(product.id)) {
            continue;
        }
    ...
    let recommendations = pickUpToTwoRandom(otherProducts);
