...
function convertPrice(from, toCode) {
    const rate = getRate(from.currencyCode, toCode);
    const [units, nanos] = applyRate(from.units, from.nanos, rate);

    return { units, nanos, currencyCode: toCode };
}
...
async function handleBusinessLogic(event, callFunction) {
    ...
    if (Array.isArray(event.prices)) {
        return {
            prices: event.prices.map((price) => convertPrice(price, event.toCode))
        };
    }

    return convertPrice(event.from, event.toCode);
}
