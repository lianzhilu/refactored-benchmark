...
async function batchDeleteItems(deleteRequests) {
    const chunks = chunkItems(deleteRequests, MAX_BATCH_WRITE_SIZE);

    for (const chunk of chunks) {
        let unprocessed = chunk;

        while (unprocessed.length > 0) {
            const response = await ddb.batchWriteItem({
                RequestItems: {
                    [CartTable]: unprocessed
                }
            }).promise();

            unprocessed = (response.UnprocessedItems && response.UnprocessedItems[CartTable]) || [];
        }
    }
}
