...
async function handleBusinessLogic(event, callFunction, callFunctionAsync) {
    ...
    switch (operation) {
        case "get":
            ...
            let productsListCurrency = productsList_response.products;
            if (productsListCurrency && productsListCurrency.length > 0) {
                let pricesToConvert = productsListCurrency.map(pr => ({ from: pr.priceUsd }));
                let convertedPrices = await callFunction("currency", {
                    prices: pricesToConvert,
                    toCode: currencyPref
                });
                
                productsListCurrency = productsListCurrency.map((pr, index) => {
                    let newPrice = convertedPrices[index];
                    pr.price = {
