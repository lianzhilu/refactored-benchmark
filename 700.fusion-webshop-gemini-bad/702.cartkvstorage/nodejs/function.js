...
const ddb = new AWS.DynamoDB({
    ...
    accessKeyId: 'None',
    secretAccessKey: 'None',
    httpOptions: { timeout: 5000, agent: new https.Agent({ keepAlive: true }) }
});
...
exports.handler = async function(event) {
    try {
        ...
        if (operation == "empty") {
            ...
            if (current.length > 0) {
                // DynamoDB batchWriteItem supports a maximum of 25 requests per batch
                const batchSize = 25;
                for (let i = 0; i < current.length; i += batchSize) {
                    const batchItems = current.slice(i, i + batchSize);
