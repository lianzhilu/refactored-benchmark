...
async function handleBusinessLogic(event, callFunction) {
    ...
    // 方案3: 优先使用调用方传入的产品列表（event.products），避免重复调用 listproducts
    // 若调用方未传入，则回退到正常网络请求
    let products;
    if (event.products && Array.isArray(event.products)) {
        products = { products: event.products };
    } else {
        products = await callFunction("listproducts", {});
    }

    let goodProducts = products.products.filter(e => event.productIds.includes(e.id));

    // 方案9: 使用 Set 替代 Array.includes，将类别查找从 O(n) 降低到 O(1)
    const goodProductIdSet = new Set(goodProducts.map(e => e.id));
    const categorySet = new Set(goodProducts.flatMap(e => e.categories));

    let otherProducts = products.products.filter(e => {
        if (goodProductIdSet.has(e.id)) {
            return false;
        }
        return e.categories.some(cat => categorySet.has(cat));
    });
