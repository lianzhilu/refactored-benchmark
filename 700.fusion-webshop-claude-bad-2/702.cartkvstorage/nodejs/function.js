...
exports.handler = async function(event) {
    try {
        ...
        if (operation == "empty") {
            ...
            let itemIds = current.Items.map(item => item.itemId.S);

            if (itemIds.length > 0) {
                // 方案4: 使用 BatchWriteItem 批量删除，每批最多 25 个 item
                // 替换原来每个 item 单独 deleteItem 的 N+1 次 DynamoDB 调用
                const batches = [];
                for (let i = 0; i < itemIds.length; i += 25) {
                    batches.push(itemIds.slice(i, i + 25));
                }
                await Promise.all(batches.map(batch => {
                    return ddb.batchWriteItem({
                        RequestItems: {
                            [CartTable]: batch.map(itemId => ({
                                DeleteRequest: {
                                    Key: {
