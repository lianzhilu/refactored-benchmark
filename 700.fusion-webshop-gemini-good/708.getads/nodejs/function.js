...
async function handleBusinessLogic(event, callFunction) {
    ...
    if (adsarray.length <= 2) return adsarray;
    let idx1 = Math.floor(Math.random() * adsarray.length);
    let idx2 = Math.floor(Math.random() * (adsarray.length - 1));
    if (idx2 >= idx1) idx2++;
    return [adsarray[idx1], adsarray[idx2]];
}
