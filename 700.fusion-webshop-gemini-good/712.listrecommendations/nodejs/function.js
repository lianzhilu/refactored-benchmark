...
async function handleBusinessLogic(event, callFunction) {
    ...
    let goodProductIdsSet = new Set(event.productIds);
    let goodProducts = products.products.filter(e => goodProductIdsSet.has(e.id));
    
    let categoriesSet = new Set();
    goodProducts.forEach(e => {
        if(e.categories) {
            e.categories.forEach(c => categoriesSet.add(c));
        }
    });
    ...
    let otherProducts = products.products.filter(e => {
        ...
        if (e.categories) {
            for (let cat of e.categories) {
                if (categoriesSet.has(cat)) return true;
            }
        }
        return false;
    });
    ...
    if (otherProducts.length <= 2) {
        ...
        let idx1 = Math.floor(Math.random() * otherProducts.length);
        let idx2 = Math.floor(Math.random() * (otherProducts.length - 1));
        if (idx2 >= idx1) idx2++;
        recommendations = [otherProducts[idx1], otherProducts[idx2]];
    }
